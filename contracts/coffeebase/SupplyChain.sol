...
contract SupplyChain is Ownable, ConsumerRole, DistributorRole, FarmerRole, RetailerRole {
  ...
  // Call to log the transaction hashes, following the comments, but does not sound to me
  // that the blockchain is the right place to store these, I would have thought an off-chain
  // storage would have better suited
  function addTransactionHistory(uint _upc, string memory txHash) public
  // To avoid spamming attacks, restricting to owner
  onlyOwner
  {
    itemsHistory[_upc].push(txHash);
  }

  function getTransactionHistory(uint _upc) public view returns (string[] memory) {
    ...
    onlyDistributor
  {
    ...
    // return any excess ether back to buyer, not a good security practice to have it as a modifier
